...
def generate_data(N, mu, beta0, beta1, sigma2, S):
    ...
    plt.xlabel('X')
    plt.ylabel('Y')
    plt.title(f'Regression Line: Y = {slope:.4f} * X + {intercept:.4f}')
    plt.legend()
    ...
    plt.axvline(slope, color="blue", linestyle="--", linewidth=1, label=f"Observed Slope: {slope:.4f}")
    plt.axvline(intercept, color="orange", linestyle="--", linewidth=1, label=f"Observed Intercept: {intercept:.4f}")
...
def hypothesis_test():
    ...
    if test_type == ">":
        ...
        diff_simulated = np.abs(simulated_stats - hypothesized_value)
        diff_observed = abs(observed_stat - hypothesized_value)
        p_value = np.mean(diff_simulated >= diff_observed)
    else:
        p_value = None
    ...
    plt.figure()
    plt.hist(simulated_stats, bins=20, alpha=0.7, color='pink', label='Simulated Statistics')
    plt.axvline(observed_stat, color='red', linestyle='--', linewidth=2, label=f'Observed {parameter.capitalize()}: {observed_stat:.4f}')
    plt.axvline(hypothesized_value, color='blue', linestyle='-', linewidth=2, label=f'Hypothesized {parameter.capitalize()} (H₀): {hypothesized_value:.4f}')
    ...
    session["p_parameter"] = parameter
    session["p_test_type"] = test_type
...
def confidence_interval():
    ...
    if ci_lower <= true_param <= ci_upper:
        includes_true = 'Yes'
    else:
        includes_true = 'No'
    ...
    plt.scatter(mean_estimate, 0, color='blue', s=80, label='Mean Estimate', zorder=5)
